// Main particles.js file for Three.js setup and particle animations
...
// Create point cloud for particles
function createPointCloud() {
    ...
    // Define colors from our palette
    const particleColors = [
        { r: colorPalette.red.r, g: colorPalette.red.g, b: colorPalette.red.b },
        { r: colorPalette.white.r, g: colorPalette.white.g, b: colorPalette.white.b },
        { r: colorPalette.metalGray.r, g: colorPalette.metalGray.g, b: colorPalette.metalGray.b },
    ];
    
    // Create helper function for clustered particles
    function createCluster(centerX, centerY, centerZ, width, height, depth, count, color, startIndex) {
        for (let i = 0; i < count && startIndex + i < pointCount; i++) {
            ...
            // Create target position for disintegration effect
    ...
    // Create a plane of particles (grid pattern) for the "floor"
    const gridSize = 1000;
    ...
    for (let x = -gridSize/2; x < gridSize/2; x += gridSpacing) {
        for (let z = -gridSize/2; z < gridSize/2; z += gridSpacing) {
            if (index < pointCount - 1) {
                ...
                positions[i3 + 1] = -200 + (Math.random() - 0.5) * 5; // Below center
                ...
                // Random color from our palette for grid points
    ...
    // Create central sphere cluster (red accent)
    index = createCluster(0, 50, 0, 200, 200, 200, pointCount * 0.2, particleColors[0], index);
    
    // Create secondary clusters (white)
    index = createCluster(-150, 0, 100, 100, 100, 100, pointCount * 0.1, particleColors[1], index);
    index = createCluster(150, 0, -100, 100, 100, 100, pointCount * 0.1, particleColors[1], index);
    
    // Create metal gray particles in a complex shape
    for (let i = 0; i < pointCount * 0.2 && index < pointCount; i++) {
        const i3 = index * 3;
        const t = i / (pointCount * 0.2);
        ...
        // Use metal gray color
        colors[i3] = particleColors[2].r;
        colors[i3 + 1] = particleColors[2].g;
        colors[i3 + 2] = particleColors[2].b;
        
        // Store original color
        originalColors.push({
            r: particleColors[2].r,
            g: particleColors[2].g,
            b: particleColors[2].b
        });
...
function updateParticles() {
    ...
    for (let i = 0; i < originalPositions.length; i++) {
        ...
        // Color transition from original colors to metallic to black
        const origColor = originalColors[i];
        
        if (disintegrationFactor < 0.5) {
            // Original to metal grey (0-0.5)
